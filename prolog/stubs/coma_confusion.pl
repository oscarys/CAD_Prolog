/* ============================================================
   MODULE: coma_confusion
   SOURCE: Churchill's Pocketbook of Differential Diagnosis,
           3rd ed., Raftery, Lim, Ostor -- Elsevier 2010, p. 67-79
   AUTHORS: <your names>
   SYSTEM:  Neurological / Metabolic

   DIAGNOSES TO ENCODE (from Churchill's):
   %   hypoglycaemia                          frequency: common
   %   drug_overdose                          frequency: common
   %   alcohol_intoxication                   frequency: common
   %   stroke                                 frequency: common
   %   meningitis                             frequency: occasional
   %   head_injury                            frequency: occasional
   %   hepatic_encephalopathy                 frequency: occasional
   %   uraemia                                frequency: occasional
   %   hypercapnia                            frequency: occasional

   SYMPTOM ATOMS (asserted by bridge as symptom/2):
   %   symptom(reduced_consciousness         , Value)  -- type: yesno
   %   symptom(confusion                     , Value)  -- type: yesno
   %   symptom(onset                         , Value)  -- type: choice: sudden|gradual
   %   symptom(fever                         , Value)  -- type: yesno
   %   symptom(head_injury                   , Value)  -- type: yesno
   ...
   %   symptom(renal_disease                 , Value)  -- type: yesno
   %   symptom(focal_neurology_history       , Value)  -- type: yesno

   FINDING ATOMS (asserted by bridge as finding/2):
   %   finding(gcs_score                     , Value)  -- type: number
   ...
   %   finding(pinpoint_pupils               , Value)  -- type: yesno
   %   finding(blood_glucose_low             , Value)  -- type: yesno
   %   finding(jaundice                      , Value)  -- type: yesno
...
:- module(coma_confusion, [diagnose/2, frequency/2,
...
frequency(drug_overdose                         ,  common).
frequency(alcohol_intoxication                  ,  common).
frequency(stroke                                ,  common).
frequency(meningitis                            ,  occasional).
frequency(head_injury                           ,  occasional).
frequency(hepatic_encephalopathy                ,  occasional).
frequency(uraemia                               ,  occasional).
frequency(hypercapnia                           ,  occasional).
